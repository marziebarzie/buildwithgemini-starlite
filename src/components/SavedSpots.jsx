  ...
  return (
    <div className="space-y-3 text-slate-100 h-full flex flex-col justify-between">
      ...
      <div className="space-y-2.5 flex-1 min-h-0 overflow-y-auto pr-1">
        {savedSpots.map((spot) => (
          <div key={spot.savedId} className="glass-card p-3 space-y-2 border-sky-500/30 hover:border-sky-500/60 transition">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-sm font-bold text-white font-heading">{spot.name}</h3>
              </div>

              <button
                onClick={() => onDeleteSpot(spot.savedId)}
                className="text-slate-400 hover:text-rose-400 p-1.5 rounded-lg bg-slate-950 hover:bg-rose-950/50 transition shrink-0"
                title="Remove spot"
              >
            ...
            <div className="bg-slate-950 p-2 rounded-lg border border-slate-800 flex items-center gap-1.5 text-[10px] font-mono text-sky-300">
              <Calendar className="w-3.5 h-3.5 text-sky-400 shrink-0" />
              <span>
                <strong>Saved Timestamp:</strong> {spot.savedTimestamp}
              </span>
            </div>

            {/* RECOMMENDED GEAR SPEC */}
            <div className="bg-sky-950/30 p-2 rounded-lg border border-sky-900/40 text-[11px] space-y-0.5">
